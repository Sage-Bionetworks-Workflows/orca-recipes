...
def main():
    ...
    for _, row in done_items.iterrows():
        ...
        subset_issues = roadmap_jira_issues[
            roadmap_jira_issues["id"].isin(delivery_tickets)
        ]
        all_epic_issues = roadmap_epic_issues[
            roadmap_epic_issues["parent"].isin(subset_issues.id)
        ]
        all_issues = pd.concat(
            [subset_issues, all_epic_issues, row.to_frame().T], ignore_index=True
        )
        all_issues = all_issues[
            ~all_issues["resolution"].isin(
                [
                    "Won't Do",
                    "Duplicate",
                    "Cancelled",
        ...
        print(
            ...
            row["key"],
            row["summary"],
            f"- {sum(~all_issues['issuetype'].isin(['Epic', 'Idea']))} issues",
        )
