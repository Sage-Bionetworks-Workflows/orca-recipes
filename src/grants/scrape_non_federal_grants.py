...
URLS_FILE = "src/grants/urls.txt"  # Path to file with URLs (one per line)
OUTPUT_FILE = "all_grants.csv"
ANTHROPIC_API_KEY = os.getenv("ANTHROPIC_API_KEY")


def load_data_model(
    syn: synapseclient.Synapse, data_model_synid: str
) -> pd.DataFrame:
...
4. Return the results in CSV format using pipe ('|') as the delimiter with these exact column headers (in order): {'|'.join(field_names)}
...
8. Ensure proper CSV escaping for fields containing pipes or quotes
    ...
    for idx, url in enumerate(urls, 1):
        ...
        if csv_result:
            ...
            try:
                ...
                df = pd.read_csv(StringIO(csv_result), sep="|")
