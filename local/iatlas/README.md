...
3. Run `load.py` to create case lists and run the cbioportal validator on your outputted files without the `upload` flag (dry run)
...
6. Run the same command with `load.py` now with the `upload` flag to upload to synapse
...
Run cbioportal validator and create the general case lists without loading into Synapse
