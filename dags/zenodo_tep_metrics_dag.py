...
import requests
from synapseclient.models import Activity, File, UsedURL
...
# Base filename for the uploaded reports
REPORT_BASE_FILENAME = "TREATAD_Target_Enabling_Metrics"

# Attached to every uploaded file version to record its provenance
SYNAPSE_VERSION_COMMENT = "Generated by the orca-recipes zenodo_tep_metrics_dag"

# GitHub location of this DAG, recorded as the "executed" step in the Synapse
# provenance (Activity) of each uploaded report
DAG_SOURCE_URL = (
    "https://github.com/Sage-Bionetworks-Workflows/orca-recipes/blob/main/"
    "dags/zenodo_tep_metrics_dag.py"
)
...
def build_provenance() -> Activity:
    """Build the Synapse provenance (Activity) for an uploaded report.

    Records where the metrics came from (the Zenodo TREAT-AD community records
    API endpoint, as a used reference) and what produced them (this DAG's
    source on GitHub, as an executed reference) so the reports in Synapse
    are traceable back to their source and code.

    Returns:
        Activity: The provenance to attach to an uploaded File
    """
    records_url = f"{ZENODO_COMMUNITIES_URL}/{TREATAD_COMMUNITY_ID}/records"
    return Activity(
        name="Zenodo TREAT-AD TEP metrics collection",
        description=(
            "Monthly TREAT-AD Target Enabling Package usage metrics (views and "
            "downloads) pulled from the Zenodo API and exported to CSV by the "
            "orca-recipes zenodo_tep_metrics_dag."
        ),
        used=[
            UsedURL(
                name="Zenodo TREAT-AD community records API",
                url=records_url,
            )
        ],
        executed=[
            UsedURL(
                name="zenodo_tep_metrics_dag source (orca-recipes)",
                url=DAG_SOURCE_URL,
            )
        ],
    )
...
def export_reports_to_synapse(
    ...
    A version comment and a Synapse Activity (provenance) are attached to each
    upload to record that it came from this DAG and which Zenodo API endpoint
    the metrics were pulled from.
    ...
    """
    base_filepath = os.path.join(os.getcwd(), REPORT_BASE_FILENAME)
    ...
    try:
        ...
        for output in outputs:
            ...
            uploaded = File(
                path=output.path,
                parent_id=folder_id,
                version_comment=SYNAPSE_VERSION_COMMENT,
                activity=build_provenance(),
