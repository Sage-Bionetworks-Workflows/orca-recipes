"""DOI to data catalog pipeline - business logic."""
...
def extract_public_dois(
    ...
    """Extract public DOIs with file sizes and download counts from Snowflake."""
    query = """
with
...
def submit_enrichment_batch(
    ...
    """Fetch wikis, build prompts, and submit a Claude batch request.
    ...
    Returns:
        Anthropic batch ID to poll and collect later. Returns empty string if
    ...
    logger.info("Claude enrichment needed for %d rows.", len(rows_to_enrich))
...
def collect_enrichment_results(
    df: pd.DataFrame,
    batch_id: str,
    anthropic_client: anthropic.Anthropic,
) -> pd.DataFrame:
    """Apply results from a completed Claude batch back onto the DataFrame.

    Args:
        df: The same merged DataFrame passed to submit_enrichment_batch.
        batch_id: Batch ID returned by submit_enrichment_batch.
        anthropic_client: Authenticated Anthropic client.
    ...
    for result in anthropic_client.messages.batches.results(batch_id):
        ...
        if result.result.type != "succeeded":
            logger.warning("Batch result for %s failed: %s", entity_id, result.result.error)
            continue
        text = next(
...
def load_to_data_catalog(
    df: pd.DataFrame,
    syn: synapseclient.Synapse,
    data_catalog_table_id: str,
    existing_datasets_table_id: str,
) -> int:
    """Filter out already-catalogued datasets and upsert new ones.

    Args:
        df: Enriched and merged DataFrame from enrich_with_claude.
        syn: Authenticated Synapse client.
        data_catalog_table_id: Synapse table ID to upsert into.
        existing_datasets_table_id: Synapse table ID to check for already-added datasets.
    ...
    table = Table(id=data_catalog_table_id)
    table.delete_rows(query=f"SELECT * FROM {data_catalog_table_id}")
    table.upsert_rows(values=filtered_df, primary_keys=["DOI_ID"])
